...
grille *creationplateau() {
  grille *plateau = (grille *)malloc(
      sizeof(grille)); // Alloue de la mémoire pour la structure grille
  if (plateau == NULL) {
...
void detruirejoueur(joueur *joueur) { free(joueur); }
void detruirepiece(piece *piece) { detruirejoueur(piece->joueur); free(piece); }

void detruireplateau(grille *plateau) {
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
      detruirepiece(plateau->grid[i][j]);
    }
  }
  free(plateau);
}
...
bool valideligne(int x, int numJoueur, int forme, grille *plateau) {
  ...
  while (i < 4 && possible == true) {
    if (plateau->grid[x][i] != NULL && plateau->grid[x][i]->formes == forme &&
        plateau->grid[x][i]->joueur->numJoueur == numJoueur) {
      possible = false;
    }
    i++;
  }
...
bool validecolonne(int y, int numJoueur, int forme, grille *plateau) {
  ...
  while (i < 4 && possible == true) {
    if (plateau->grid[i][y] != NULL && plateau->grid[i][y]->formes == forme &&
        plateau->grid[i][y]->joueur->numJoueur == numJoueur) {
      possible = false;
    }
    i++;
  }
...
bool valideregion(int x, int y, int numJoueur, int forme, grille *plateau) {
  ...
  for (int i = 2 * sectionx; i < subsectionx + (sectionx * 2); i++) {
    for (int j = 2 * sectiony; j < subsectiony + (sectiony * 2); j++) {
      if (plateau->grid[i][j] != NULL && plateau->grid[i][j]->formes == forme &&
          plateau->grid[i][j]->joueur->numJoueur == numJoueur) {
        possible = false;
      }
    }
  }
...
bool valide(int forme, int numJoueur, int x, int y, grille *plateau) {
  if (plateau->grid[x][y] != NULL) {
    ...
    return valideligne(x, numJoueur, forme, plateau) &&
           validecolonne(y, numJoueur, forme, plateau) &&
           valideregion(x, y, numJoueur, forme, plateau);
  }
}

grille *ajoutpiece(grille *plateau, int forme, joueur *joueur, int x, int y) {
  if (valide(forme, joueur->numJoueur, x, y, plateau)) {
    if (joueur->piecerestante[forme - 1][1] == 0) {
      printf("Vous n'avez plus de piece de cette forme\n");
      return plateau;
    }
    else
    {
       piece *pieceajoutee = (piece *)malloc(sizeof(piece));
    pieceajoutee->formes = forme;
    pieceajoutee->joueur = joueur;
  ...
  for (int i = 0; i < 4; i++) {
    ...
    for (int j = 0; j < 4; j++) {
      if (plateau->grid[i][j] == NULL) {
        ...
        printf("\033[34m(%d %d)\033[00m",
               plateau->grid[i][j]->joueur->numJoueur,
               plateau->grid[i][j]->formes);
      }
    }
    printf("|\n");
  }
...
bool victoirecolonne(int y, grille *plateau) {
  ...
  while (j < 4 && victoire == false) {
    if (plateau->grid[j][y] != NULL &&
        estdanslaliste(plateau->grid[j][y], lparcouru) == 0) {
...
bool chercheregion(int x, int y, grille *plateau, list *lparcouru) {
  ...
  for (int i = 2 * sectionx; i < subsectionx + (sectionx * 2); i++) {
    for (int j = 2 * sectiony; j < subsectiony + (sectiony * 2); j++) {
      if (plateau->grid[i][j] != NULL &&
          estdanslaliste(plateau->grid[i][j], lparcouru) == 0) {
...
bool victoire(grille *plateau, int x, int y) {
  bool victoire = false;
  if (victoireligne(x, plateau) == true ||
      victoirecolonne(y, plateau) == true ||
      victoireregion(x, y, plateau) == true) {
    victoire = true;
  }
...
grille *demandepiece(grille *plateau, joueur *joueur) {
  ...
  scanf("%d", &y);
  if (pieceajoutee == NULL) {
    return plateau;
  }
  {
    plateau = ajoutpiece(plateau, pieceajoutee->formes, joueur, x, y);
...
bool jeuencours1VSIA() {
  ...
  if (plateau == NULL || joueur1 == NULL || joueurIA == NULL)
  {
    return false;
  }
