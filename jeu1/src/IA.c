...
bool victoiretouteregion(grille *plat) {
  if (victoireregion(0, 0, plat) || victoireregion(3, 0, plat) ||
      victoireregion(0, 3, plat) || victoireregion(3, 3, plat))
    return true;
...
int aligne(grille *plateau, int n) {
  ...
  for (int x = 0; x < 4; x++) {
    for (int y = 0; y < 4; y++) {
      score += aligneregion(x, y, plateau, n) + aligneligne(x, plateau, n) +
               alignecolonne(y, plateau, n);
    }
  }
  return score;
}
...
int evaluation(grille *plat, int profondeur) {
  ...
  if (victoiretouteligne(plat) || victoiretoutecolonne(plat) ||
      victoiretouteregion(plat)) {
    if (paire)
      return 100000;
    else
      return -100000;
...
grille *grillecopie(grille *acopier) {
  ...
  for (int i = 0; i < 4; i++)                       // parcours de la grille
  {
    for (int j = 0; j < 4; j++) {
      ...
      {
        copie->grid[i][j] = (piece *)malloc(sizeof(piece)); // on alloue la
                                                            // piece
        *(copie->grid[i][j]) =
            *(acopier->grid[i][j]); // on copie la piece dans la copie
      } else {
        copie->grid[i][j] = NULL;
      }
    }
...
arbre *generecoup(joueur *jo1, joueur *jo2, grille *plat, int profondeur,
                  int profondeurmax) {
...
int minmaxalphabeta(arbre *noeud, int profondeur, int maximizingPlayer,
                    int alpha, int beta) {
  if (profondeur == 0) {
    return evaluation(noeud->plateau, profondeur);
  }
  ...
  if (maximizingPlayer) {
    ...
    for (int i = 0; i < N && noeud->fils[i] != NULL; i++) {
      int eval =
          minmaxalphabeta(noeud->fils[i], profondeur - 1, 0, alpha, beta);
      if (eval > maxEval) {
        maxEval = eval;
      }
      if (eval > alpha) {
    ...
    for (int i = 0; i < N && noeud->fils[i] != NULL; i++) {
      int eval =
          minmaxalphabeta(noeud->fils[i], profondeur - 1, 1, alpha, beta);
      if (eval < minEval) {
        minEval = eval;
      }
      if (eval < beta) {
