...
void display(SDL_Texture* bg_texture, cadrillage* cadri, SDL_Renderer* renderer) {
    ...
    for (int i = 0; i < GRID_CARTE_WIDTH; ++i) {
        ...
        {
            if (cadri->emplacement[i][j]) {
                SDL_RenderCopy(renderer, cadri->emplacement[i][j]->texture, NULL, &cadri->emplacement[i][j]->rect);
                printf("Carte %d : x = %d, y = %d, w = %d, h = %d\n", i, cadri->emplacement[i][j]->rect.x, cadri->emplacement[i][j]->rect.y, cadri->emplacement[i][j]->rect.w, cadri->emplacement[i][j]->rect.h);
            }   
            else {
...
void phase_de_pioche(SDL_Renderer* renderer, tab_carte* cartes, cadrillage* cadri, SDL_Texture* bg_texture, sitjoueur* joueur1, sitjoueur* joueur2) {
    bool quit = false;
    SDL_Event event;
    while (!quit)
    {
        ...
        while (nbr_carte_liste(joueur1->mainjoueur,5)<=joueur1->nb_cartes_mainjoueur)
        {
        piocher(joueur1,joueur2,1);
        deplacer_texture(renderer, bg_texture, cartes->tab_carte[12]->texture, cartes->tab_carte[12]->rect, cadri->emplacement[1][2]->rect, 10);
...
void phase_de_construction(SDL_Renderer* renderer, tab_carte* cartes, cadrillage* cadri, SDL_Texture* bg_texture, sitjoueur* joueur1, sitjoueur* joueur2) {
    bool quit = false;
    SDL_Event event;
    while (!quit)
    {
        while (SDL_PollEvent(&event)) {
            ...
            else if (event.type == SDL_MOUSEBUTTONDOWN && event.button.button == SDL_BUTTON_LEFT)
            {
                ...
                if (carte_info->troisieme != -1)
                {
                    ...
                    {
                        deplacer_texture(renderer, bg_texture, cartes->tab_carte[carte_info->troisieme]->texture, cadri->emplacement[carte_info->premier][carte_info->deuxieme]->rect, cadri->emplacement[carte_info->premier][4]->rect, 10);
                        positioner_carte_constru_joueur(cadri, cartes, joueur1, joueur2,carte_info->troisieme);
...
void phase_de_echange(SDL_Renderer* renderer, tab_carte* cartes, cadrillage* cadri, SDL_Texture* bg_texture, sitjoueur* joueur1, sitjoueur* joueur2) {
    bool quit = false;
    SDL_Event event;
...
void tour_de_jeu(SDL_Renderer* renderer,tab_carte* cartes, cadrillage* cadri,  SDL_Texture* bg_texture, sitjoueur* joueur1, sitjoueur* joueur2) {
    ...
    while (!quit) {
        while (SDL_PollEvent(&event)) {
            ...
            phase_de_construction(  renderer, cartes,cadri, bg_texture, joueur1, joueur2);
            ...
            phase_de_construction(  renderer, cartes,cadri, bg_texture, joueur2, joueur1);
