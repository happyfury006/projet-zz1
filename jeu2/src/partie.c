...
void phase_echange(sitjoueur *joueur1, sitjoueur *joueur2) {
  ...
  if (choix == 0) {
    ...
    for (int i = 0; i < nb_echange; i++) {
      ...
      printf(
          "Quel carte de votre main voulez vous echanger avec le marche ?\n");
      int choix_main = 0;
      scanf("%d", &choix_main);
      if (joueur1->mainjoueur[choix_main] > 0) {
...
void phase_pioche(sitjoueur *joueur1, sitjoueur *joueur2) {
  // joueur1
  piocher(joueur1, joueur2,
          joueur1->nb_cartes_mainjoueur -
              nbr_carte_liste(joueur1->mainjoueur, 5));
  // joueur2
  // piocher(joueur2,joueur1,joueur2->nb_cartes_mainjoueur-
  // nbr_carte_liste(joueur2->mainjoueur,5));
}
