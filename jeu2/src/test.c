...
// J'aimerai avoir une fonction qui fait que lorsque je clique sur un SDL rect
// qui se trouve en x = 0 et y = 0 alors on incrémente un compteur de 1 que l'on
// initialise à 0 (je veux l'afficher sur la fenêtre)
...
void mouvement_jouer_carte(SDL_Renderer *renderer,
...
void display_menu(SDL_Texture *bg_menu_texture, SDL_Texture *button_texture,
                  SDL_Renderer *renderer, SDL_Rect *button_rect) {
...
int main(int argc, char *argv[]) {
  ...
  (void)argv;
  SDL_Window *window =
      SDL_CreateWindow("Échanger Positions", SDL_WINDOWPOS_CENTERED,
                       SDL_WINDOWPOS_CENTERED, 800, 600, 0);
  SDL_Renderer *renderer = SDL_CreateRenderer(
      window, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
  SDL_Texture *background_texture =
      IMG_LoadTexture(renderer, "../Images/fondblanc.png");
  ...
  SDL_Texture *bg_menu_texture =
      IMG_LoadTexture(renderer, "../Images/background.png");

  SDL_Texture *button_texture =
      IMG_LoadTexture(renderer, "../Images/Start.png");
  SDL_Rect button_rect = {WINDOW_LARGEUR / 2 - BUTTON_WIDTH / 2,
                          WINDOW_HAUTEUR / 2 - BUTTON_HEIGHT / 2, BUTTON_WIDTH,
                          BUTTON_HEIGHT};
    ...
    while (SDL_PollEvent(&event)) {
      if (event.type == SDL_QUIT) {
        quit = true;
                 ...
                 event.button.button == SDL_BUTTON_LEFT) {
        echanger_positions(renderer, background_texture, textures, rectangles,
                           2);
