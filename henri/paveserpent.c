...
void draw(SDL_Renderer *renderer, float w, float h, int color1,
          int color2) { // Je pense que vous allez faire moins laid :)

  SDL_Rect rectangle;
  SDL_Rect rectangle2;
  SDL_Rect carre;

  SDL_SetRenderDrawColor(renderer, color1, 0,
  ...
  rectangle2.h = 20;
  rectangle2.x = (rectangle.x + (rectangle.w / 2)) -
                 rectangle2.w / 2;           // x haut gauche du rectangle
  ...
  /*SDL_RenderDrawLine(renderer,
          0, 0,                                          // x,y du point de la
     première extrémité 400, 400);     */                                // x,y seconde extrémité

  SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);

  /* tracer un cercle n'est en fait pas trivial, voilà le résultat sans algo
  intelligent ... for (float angle = 0; angle < 2 * M_PI; angle += M_PI / 4000)
  { SDL_SetRenderDrawColor(renderer, (cos(angle * 2) + 1) * 255 / 2,          //
  quantité de Rouge (cos(angle * 5) + 1) * 255 / 2,          //          de vert
