...
void render_background(SDL_Texture *bg_texture, SDL_Window *window, SDL_Renderer *renderer, int offset) {
    SDL_Rect 
    ...
    SDL_QueryTexture(bg_texture, NULL, NULL, &source.w, &source.h);          // Récupération des dimensions de l'image

    destination = window_dimensions;      // On fixe les dimensions de l'affichage à celles de la fenêtre

    destination.x = -offset;              // Appliquer l'offset pour créer l'effet de défilement

    SDL_RenderCopy(renderer, bg_texture, &source, &destination); // Création de l'élément à afficher
...
void play_with_texture_4(SDL_Texture* bg_texture, SDL_Texture* my_texture, SDL_Window* window, SDL_Renderer* renderer) {
    SDL_Rect 
    ...
    int nb_images = 8;                     // Il y a 8 vignettes dans la ligne de l'image qui nous intéresse
