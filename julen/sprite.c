...
void play_with_texture_3(SDL_Texture* bg_texture, SDL_Texture* anakin_texture,SDL_Texture* sebulba_texture,SDL_Texture* destroyer_texture, SDL_Window* window, SDL_Renderer* renderer) {
    ...
    SDL_QueryTexture(anakin_texture, NULL, NULL, &source_anakin.w, &source_anakin.h);
    SDL_QueryTexture(sebulba_texture, NULL, NULL, &source_sebulba.w, &source_sebulba.h);
    SDL_QueryTexture(destroyer_texture, NULL, NULL, &source_destroyer.w, &source_destroyer.h);
   ...
   //Initialisation de la position de l'image de anakin
    ...
    destination_anakin.x = (window_dimensions.w - destination_anakin.w) / 2; // Center horizontally
   ...
   //Initialisation de la position de l'image de sebulba
      ...
      destination_sebulba.x = (window_dimensions.w - destination_sebulba.w) *2; // Center horizontally
   ...
   //Initialisation de la position de l'image de destroyer
      ...
      destination_destroyer.x = (window_dimensions.w - destination_destroyer.w) ; // horizontally Right
   ...
   //  int initial_y = destination_anakin.y;
    ...
    while (running) {
        ...
        // Animation vaiseau anakin en petit rectangle au tour de sa position initiale qui change
        switch (direction) {
            case 0: // Move right
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (destination_bg[i].x <= -destination_bg[i].w) {
                destination_bg[i].x = destination_bg[(i + 2) % 3].x + destination_bg[(i + 2) % 3].w;
            }
        }
      ...
      //   SDL_SetTextureAlphaMod(anakin_texture,  0);
        ...
        SDL_RenderCopy(renderer, destroyer_texture, &source_destroyer, &destination_destroyer);
        SDL_RenderCopy(renderer, sebulba_texture, &source_sebulba, &destination_sebulba);
        SDL_RenderCopy(renderer, anakin_texture, &source_anakin, &destination_anakin);
...
int main(int argc, char** argv) {
    ...
    window = SDL_CreateWindow("Texture Animation",
